%THIS GRAPHS DISTRIBUTION OF INITIAL DATA. IT IS NOT USED IN THE ACTUAL
...
Dist = zeros(2*bound*(1/mesh)+1,2*bound*(1/mesh)+1);
    ...
    for i=1:(2*bound*(1/mesh)+1)
        psiMR = abs(psiMinusR((i-(bound*(1/mesh)+1))*mesh,theta,sigma,k,mu)).^2;
        psiPR = abs(psiPlusR((i-(bound*(1/mesh)+1))*mesh,theta,sigma,k,mu)).^2;
        Dist(i) = j0(psiMR,psiPR);
    end
