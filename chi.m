...
if t>=(2*n+1)*L-(sign*s)
    ...
    if sign*((-1)^n)>0
        ...
        a2 = -(omega/2)*integral2(@(xi,sigma) besselfun(radical(t-xi,L,sigma),omega,1).*(t-xi-L+sigma).*psiPlusR(sigma,theta,alpha,k,mu).*R(0,sign*s,xi,omega,L,n),((2*n+1)*L-(sign*s)),t,@(xi) L-t+xi,@(xi) L+t-xi);
